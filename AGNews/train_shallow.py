...
model = ConvNet(True, WINDOW = 3, FEATURES=2)
...
for epoch in range(1,NUMEPOCHS+1):
    ...
    if val_loss <= min_val_loss:
        ...
        torch.save(model.state_dict(), 'fasttext/sw4-2/Model_best_val_quicksave.pt')
    ...
    print('epoch [{}/{}], loss:{:.7f}, train acc:{:.4f}, val loss:{:.7f}, val acc:{:.4f} in {}h {}m {}s'.format(epoch, NUMEPOCHS,
                                                                               ...
                                                                               time_el%60))
    if epoch%5 ==0:
        torch.save(model.state_dict(), 'fasttext/sw4-2/Model_quicksave'+str(epoch)+'.pt')
...
plt.xlabel('Epochs')
plt.ylabel('Loss')
plt.savefig('fasttext/train_curve_s4-2.png')
